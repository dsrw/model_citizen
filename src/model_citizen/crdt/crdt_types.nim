...
type
  ...
  VectorClock* = ref object ## Simple vector clock for causality tracking
...
proc update*(self: VectorClock, other: VectorClock) =
  ...
  for peer_id, peer_time in other.clocks:
    if peer_id != self.local_id:
      self.clocks[peer_id] =
        max(self.clocks.get_or_default(peer_id, 0), peer_time)
...
# CRDT types
type CrdtZenSet*[T] = ref object of ZenObject[HashSet[T], T]
...
type
  ...
  ConflictResolver*[T] =
    proc(local, remote: T, local_clock, remote_clock: VectorClock): T {.gcsafe.}
